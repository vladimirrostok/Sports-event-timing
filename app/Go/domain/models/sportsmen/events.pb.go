...
func (m *SportsmenCreatedEvent) MarshalToSizedBuffer(dAtA []byte) (int, error) {
	...
	if len(m.LastName) > 0 {
		...
		i = encodeVarintEvents(dAtA, i, uint64(len(m.LastName)))
		i--
		dAtA[i] = 0x22
	}
	if len(m.FirstName) > 0 {
		...
		i = encodeVarintEvents(dAtA, i, uint64(len(m.FirstName)))
		i--
		dAtA[i] = 0x1a
	}
	if m.StartNumber != 0 {
		i = encodeVarintEvents(dAtA, i, uint64(m.StartNumber))
		i--
		dAtA[i] = 0x10
	}
...
func (m *SportsmenCreatedEvent) Size() (n int) {
	...
	if m.StartNumber != 0 {
		n += 1 + sovEvents(uint64(m.StartNumber))
	}
	l = len(m.FirstName)
	if l > 0 {
		n += 1 + l + sovEvents(uint64(l))
